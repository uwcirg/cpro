...
studySpecificTablesToWriteToProd="alerts clinics conditions items options pages projects projects_questionnaires questionnaires questions scales sites subscales targets teaching_tips teaching_tips_percentages"
today=`date +"%m%d%y"`
devDbConfig='../config/database.php'
devDbName=`grep -m 1 "'database' =>" ${devDbConfig} | cut -f4 -d"'"`
devStructTodayFileName="../db/${devDbName}_struct_${today}.sql"
devStructToCommitFileName="../db/${devDbName}_db_struct.sql"
devSystemTablesToCommitAndWriteToProd="../db/${devDbName}_system_tables.sql"
devStudySpecificTablesToCommitAndWriteToProd="../db/${devDbName}_study_specific_tables.sql"
...
prodStructTodayFileName="../db/${prodDbName}_struct_${today}"".sql"
prodSystemTables="../db/${prodDbName}_system_tables_${today}.sql"
prodStudySpecificTables="../db/${prodDbName}_study_specific_tables_${today}.sql"
prodTodayFileName="../db/${prodDbName}_${today}.sql"
...
--skip-comments --ignore-table=${prodDbName}.tickets | sed 's/AUTO_INCREMENT=[0-9]*\b//' > ${prodStructTodayFileName}
...
structDiff=`diff -b ${devStructTodayFileName} ${prodStructTodayFileName}`
...
systemDiff=`diff -b ${devSystemTablesToCommitAndWriteToProd}.tokenized ${prodSystemTables}.tokenized`
...
studySpecificDiff=`diff -b ${devStudySpecificTablesToCommitAndWriteToProd}.tokenized ${prodStudySpecificTables}.tokenized`
