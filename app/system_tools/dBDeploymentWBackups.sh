...
# This will be deployment specific, eg sme will use teaching_tips, while p3p will use p3p_teachings. Most tables will be the same though.
# paintracker
studySpecificTables="alerts sites conditions items options pages projects projects_questionnaires questionnaires questions scales subscales targets"
# esrac
#studySpecificTables="alerts clinics sites conditions items options pages projects projects_questionnaires questionnaires questions scales subscales targets teaching_tips teaching_tips_percentages"
# p3p 
#studySpecificTables="alerts conditions items options pages projects projects_questionnaires questionnaires questions scales subscales targets p3p_teachings"

# Tables which are identical across dhair2 instances 
# You prob won't need to modify this line
tablesWCommonData="acos aros aros_acos"

# destDomainRoot eg 'p3p.cirg.washington.edu', 'esrac.cirg.washington.edu'
destDomainRoot='paintracker.cirg.washington.edu'
...
sourceCommonDataTables="../db/commonData.sql"
sourceStudySpecificTables="../db/${sourceDbName}_study_specific_tables.sql"
...
destSystemTables="../db/${destDbName}_commonDhairData_tables_${today}.sql"
...
mysqldump --no-data -u ${sourceUser} -p${sourcePw} ${sourceDbName} --skip-comments --skip-extended-insert --ignore-table=${sourceDbName}.tickets | sed 's/AUTO_INCREMENT=[0-9]*\b//' > ${sourceStructTodayFileName} 
...
--skip-comments --skip-extended-insert --ignore-table=${destDbName}.tickets | sed 's/AUTO_INCREMENT=[0-9]*\b//' > ${destStructTodayFileName}
...
echo
echo "Exporting ${sourceDbName} and ${destDbName} commonDhairData tables, and diffing them."
echo
sleep 5 

mysqldump -u ${sourceUser} -p${sourcePw} --skip-extended-insert ${sourceDbName} ${tablesWCommonData} > ${sourceCommonDataTables} 

mysqldump -u ${destUser} -p${destPw} --skip-extended-insert ${destDbName} ${tablesWCommonData} > ${destSystemTables} 

sed -e 's/(/\n(/g' ${sourceCommonDataTables} > ${sourceCommonDataTables}".tokenized"
sed -e 's/(/\n(/g' ${destSystemTables} > ${destSystemTables}".tokenized"

commonDataDiff=`diff -b ${sourceCommonDataTables}.tokenized ${destSystemTables}.tokenized`
commonDataDiff='Tokenized result of commonDhairData tables diff (output files are: '${sourceCommonDataTables}' & '${destSystemTables}'):'${commonDataDiff}

printf %s\n "${commonDataDiff}" > "../db/commonDataDiffTemp.txt"
printf %s\n "${commonDataDiff}"
echo
echo "(Also output to commonDataDiffTemp.txt)"
echo
...
mysqldump -u ${sourceUser} -p${sourcePw} --skip-extended-insert ${sourceDbName} ${studySpecificTables} > ${sourceStudySpecificTables} 

mysqldump -u ${destUser} -p${destPw} --skip-extended-insert ${destDbName} ${studySpecificTables} > ${destStudySpecificTables} 

sed -e 's/(/\n(/g' ${sourceStudySpecificTables} > ${sourceStudySpecificTables}".tokenized"
sed -e 's/(/\n(/g' ${destStudySpecificTables} > ${destStudySpecificTables}".tokenized"

studySpecificDiff=`diff -b ${sourceStudySpecificTables}.tokenized ${destStudySpecificTables}.tokenized`
studySpecificDiff='Tokenized result of study-specific tables diff (output files are: '${sourceStudySpecificTables}' & '${destStudySpecificTables}'):'${studySpecificDiff}
...
echo "Commence with svn commit of ${sourceStructToCommitFileName}, ${sourceCommonDataTables}, and ${sourceStudySpecificTables}? Note that this will not update ${destDbName} - that option follows."
...
echo
echo "Also update the ${destDbName} DB with ${sourceCommonDataTables} and ${sourceStudySpecificTables}?"
select yn_dest_update in "Yes" "No"; do
    case $yn_dest_update in
        Yes )
            writtenToDestMsg=" ${destDbName} database updated."
            break;;
        No )
            writtenToDestMsg=" ${destDbName} database not modified."
            break;;
    esac
done
...
mysqldump --no-data -u ${sourceUser} -p${sourcePw} ${sourceDbName} --skip-extended-insert > ${sourceStructToCommitFileName} 
#| sed 's/Dump completed =[0-9]*\b//' 

msg=`svn commit -m "${sourceDbName} and ${destDbName} DB structures match. Committing $sourceStructToCommitFileName schema, ${sourceCommonDataTables} ($tablesWCommonData) data & struct, and ${sourceStudySpecificTables} ($studySpecificTables) data & struct. ${writtenToDestMsg}. Other comments: ${comment}" "$sourceStructToCommitFileName" "$sourceCommonDataTables" "$sourceStudySpecificTables"`
if [ -z $msg ]; then
    echo "Nothing to commit for $sourceStructToCommitFileName or $sourceCommonDataTables" 
else
    echo "${msg}"
fi
...
if [ "$yn_dest_update" = "Yes" ] ; then
    echo
    echo "Replacing ${destDbName}'s commonData and study-specific tables w/ current ${sourceDbName} snapshot..."
    # replace dest commonData tables w/ source commonData tables
    mysql -u ${destUser} -p${destPw} ${destDbName} < ${sourceCommonDataTables} 
    # replace dest study-specific tables w/ source ones
    mysql -u ${destUser} -p${destPw} ${destDbName} < ${sourceStudySpecificTables} 
fi
