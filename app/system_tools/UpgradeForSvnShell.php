 ...
 *
 * NOTE: This was modified by Justin McReynolds to perform svn commands
 */
 ...
 * @package       Cake.Console.Command
 */
class UpgradeForSvnShell extends AppShell {
 ...
 */
	public function startup() {
		parent::startup();
        //$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'params: %s', print_r($this->params, true)));
		if ($this->params['dry-run']) {
			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. '<warning>Dry-run mode enabled!</warning>'), 1, Shell::QUIET);
		}
		if (!is_dir('.svn')) {
		//if ($this->params['svn'] && !is_dir('.svn')) {
			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. '<warning>No svn repository detected!</warning>'), 1, Shell::QUIET);
		}
	}
 ...
 */
	public function all() {
		foreach ($this->OptionParser->subcommands() as $command) {
			...
			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Running %s', $name));
			$this->$name();
		}
	}
...
/*
// removed this as its chdir was problematic
		$cwd = getcwd();
		...
		if (is_dir('plugins')) {
			...
			foreach ($plugins as $plugin) {
				...
				$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Upgrading locations for plugin %s', $plugin));
				$this->locations();
			}
			$this->_files = array();
			chdir($cwd);
...
*/
        $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Upgrading locations for app directory'));
		$moves = array(
			'config' => 'Config',
        ...
        $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'just before moving anything, heres getcwd():  %s)', getcwd()));
		foreach ($moves as $old => $new) {
			if (is_dir($old)) {
				//$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Moving %s to %s', $old, $new));
//				$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'is_dir(old = %s), so creating %s, which will obviate %s', $old, $new, $old));
				if (!$this->params['dry-run']) {
					//if ($this->params['svn']) {
						$svnexec = 'svn mkdir --parents ' . escapeshellarg($new);
//			            $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'next: %s', $svnexec));
						exec($svnexec);
                        // svn move on dir not nice to contents
						/*exec('svn mv ' . escapeshellarg($old) . ' ' . escapeshellarg($old . '__'));
						exec('svn mv ' . escapeshellarg($old . '__') . ' ' . escapeshellarg($new));*/
					/**} else {
...
//                $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'not creating %s, because !is_dir(old dir %s in %s)', $new, $old, dirname($old)));
            }
		}
 ...
 */
	protected function _moveViewFiles() {
		if (!is_dir('View')) {
			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. '!is_dir(View), returning'));
			return;
		}

		$dirs = scandir('View');
		foreach ($dirs as $old) {
//			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'iterating over old dirs, heres one: %s', $old));
...
//			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'svn mdkir\'ing %s', $new));
			//$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Moving %s to %s', $old, $new));
			if (!$this->params['dry-run']) {
				//if ($this->params['svn']) {
					$svnexec = 'svn mkdir ' . escapeshellarg($new);
//			        $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'next: %s', $svnexec));
					exec($svnexec);
					/**exec('svn mv ' . escapeshellarg($old) . ' ' . escapeshellarg($old . '__'));
					exec('svn mv ' . escapeshellarg($old . '__') . ' ' . escapeshellarg($new));*/
 ...
 */
	protected function _moveAppClasses() {
		...
		foreach ($files as $old => $new) {
			if (file_exists($old)) {
				//$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Moving %s to %s', escapeshellarg($old), escapeshellarg($new)));

				if ($this->params['dry-run']) {
					continue;
				}
				//if ($this->params['svn']) {
					$svnexec = 'svn mv ' . escapeshellarg($old) . ' ' . escapeshellarg($new);
//			        $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'next: %s', $svnexec));
					exec($svnexec);
					//exec('svn mv ' . escapeshellarg($old) . ' ' . escapeshellarg($old . '__'));
					//exec('svn mv ' . escapeshellarg($old . '__') . ' ' . escapeshellarg($new));
...
//        $this->out(__d('cake_console', __FUNCTION__ . '(*); '. '_movePhpFiles for path %s', $path));
		if (!is_dir($path)) {
			return;
		}

        // kludgy...
        if ($path == 'vendors/shells'){
	        $svnexec = 'svn add Console/Command';
//            $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'next, special case for new vendors/shells location: %s', $svnexec));
		    exec($svnexec);
        }
...
//            $this->out(__d('cake_console', __FUNCTION__ . '(*); '. '_movePhpFiles for path %s, recursively', $path));
			$this->_findFiles('php');
			$filesTemp = $this->_files;
			$this->_findFiles('ctp');
            $this->_files = array_merge($filesTemp, $this->_files); 
		} else {
//            $this->out(__d('cake_console', __FUNCTION__ . '(*); '. '_movePhpFiles for path %s, non-recursively', $path));
...
//		    $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'iterating over files, heres the current: %s', $file));
			$file = $cwd . DS . $file;
			...
			$dir = dirname($new);
			if (!is_dir($dir)) {
//			    $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'This dir does not exist, so will svn mkdir it:  %s', escapeshellarg($dir)), 1, Shell::VERBOSE);
			    $svnexec = 'svn mkdir --parents ' . escapeshellarg($dir);
//			    $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'next: %s', $svnexec));
			    exec($svnexec);
				//new Folder($dir, true);
			}

			//$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Moving %s to %s', escapeshellarg($file(, escapeshellarg($new)), 1, Shell::VERBOSE);
			if (!$this->params['dry-run']) {
				//if ($this->params['svn']) {
					$svnexec = 'svn mv ' . escapeshellarg($file) . ' ' . escapeshellarg($new);
//			        $this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'next: %s', $svnexec));
					exec($svnexec);
					//exec('svn mv ' . escapeshellarg($file) . ' ' . escapeshellarg($file . '__'));
					//exec('svn mv ' . escapeshellarg($file . '__') . ' ' . escapeshellarg($new));
				/**} else {
					rename($file, $new);
				}*/
			}
 ...
 */
	protected function _filesRegexpUpdate($patterns) {
		...
		foreach ($this->_files as $file) {
//			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Updating %s...', $file), 1, Shell::VERBOSE);
...
//			$this->out(__d('cake_console', __FUNCTION__ . '(*); '. ' * Updating %s', $pattern[0]), 1, Shell::VERBOSE);
			$contents = preg_replace($pattern[1], $pattern[2], $contents);
		}

//		$this->out(__d('cake_console', __FUNCTION__ . '(*); '. 'Done updating %s', $file), 1);
		if (!$this->params['dry-run']) {
	...
	public function getOptionParser() {
		$subcommandParser = array(
			'options' => array(
				'plugin' => array(
					'short' => 'p',
					'help' => __d('cake_console', __FUNCTION__ . '(*); '. 'The plugin to update. Only the specified plugin will be updated.')
				),
				'ext' => array(
					'short' => 'e',
					'help' => __d('cake_console', __FUNCTION__ . '(*); '. 'The extension(s) to search. A pipe delimited list, or a preg_match compatible subpattern'),
					'default' => 'php|ctp|thtml|inc|tpl'
				),
                /** hard-coded in
				'svn' => array(
					'short' => 'g',
					'help' => __d('cake_console', __FUNCTION__ . '(*); '. 'Use svn command for moving files around.'),
					'boolean' => true
				),
                */
				'dry-run' => array(
					'short' => 'd',
					'help' => __d('cake_console', __FUNCTION__ . '(*); '. 'Dry run the update, no files will actually be modified.'),
					'boolean' => true
				)
			)
		);
